...
const GlobalHeader = ({ isLive, setIsLive, currentTime }) => {
  ...
  return (
    <header className="bg-gray-800 border-b border-gray-700 px-4 py-3 flex items-center justify-between fixed top-0 left-0 right-0 z-50">
      {/* Left: Logo & App Name */}
...
const LeftPanel = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <>
      {/* Toggle Button */}
      <button
        onClick={() => setIsOpen(!leftPanelOpen)}
        className="fixed left-4 top-20 bg-gray-700 text-white p-2 rounded-lg z-50 hover:bg-gray-600 transition-colors"
      >
        <Menu className="w-5 h-5" />
      </button>

      {/* Left Panel */}
      <div className={`fixed left-0 top-16 h-[calc(100vh-4rem)] bg-gray-800 border-r border-gray-700 transition-transform duration-300 z-40 ${
...
const LiveStatus = () => {
  ...
  // Sample train data for different tracks - UNCHANGED
  const trainData = {
    track1: [
  ...
  return (
    <div className="min-h-screen bg-gray-900 font-sans">
      {/* Global Header */}
      <GlobalHeader isLive={isLive} setIsLive={setIsLive} currentTime={masterClock} />
      
      {/* Left Panel */}
      <LeftPanel isOpen={leftPanelOpen} setIsOpen={setLeftPanelOpen} />

      {/* Main Content - Your Original Design UNCHANGED */}
      <div className="min-h-screen bg-gray-900 p-4 font-sans pt-20">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-gray-800 rounded-lg p-4">
            ...
            {/* YOUR ORIGINAL MAP CONTAINER - COMPLETELY UNCHANGED */}
